package com.cea.celibrary.customview.VoiceRecord;
    ...
    public int getVoiceDuration() {
        try {
            if (mFileName != null) {
                ...
                mediaPlayer.setDataSource(context, Uri.parse(mFileName));
                mediaPlayer.prepare();
                L.d(mFileName + " , " + mediaPlayer.getDuration());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void finishRecord() {
        ...
        if (intervalTime < MIN_INTERVAL_TIME) {
            ...
            if (finishedListener != null)
                finishedListener.onFinishedRecord(null, 0);
            return;
        }
...
//		viewFlipper.setDisplayedChild(2);
        Message msg = new Message();
        msg.what = 7;
        volumeHandler.sendMessageDelayed(msg, 8);
        if (finishedListener != null)
            finishedListener.onFinishedRecord(mFileName, getVoiceDuration());
    }
